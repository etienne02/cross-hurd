...
download_python_setuptools_scm() {
  if [ -d $PYTHON_SETUPTOOLS_SCM_SRC ]; then
    return 0
  fi
  wget $PYTHON_SETUPTOOLS_SCM_URL &&
  mv "v${PYTHON_SETUPTOOLS_SCM_VERSION}.tar.gz" $PYTHON_SETUPTOOLS_SCM_PKG
  unpack xf $PYTHON_SETUPTOOLS_SCM_PKG $PYTHON_SETUPTOOLS_SCM_SRC
}
...
download_gyp() {
  download_from_git $GYP_SRC $GYP_URL
    pushd $GYP_SRC &&
    apply_patch $SCRIPT_DIR/patches/gyp/0001-Fix-Python-compat-and-remove-six.patch 1 &&
    popd
}
