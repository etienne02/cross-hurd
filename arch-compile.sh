...
install iana-etc &&
	...
	install jansson &&
	#install elfutils.bootstrap && # unused variable need to "allow this error"
	...
	install gpgme &&  # needs gpg to be installed
	install libarchive &&
	install mpdecimal &&
	install libffi &&
	...
	install libedit &&
	install dmidecode &&
	install parted &&  # fails, incompatible pointer types
	install libdaemon &&
